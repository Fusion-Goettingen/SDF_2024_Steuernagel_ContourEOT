...
class TrackerRM:
    ...
    def __init__(self,
        ...
        :param scaling_factor: Scaling factor to be used, for ellipse surface measurements (original model), =1/4
    ...
    def update(self, Z):
        if self.V is None:
            # initial step:
            #   use kinematics as passed during prior
            #   use first set of measurements to initialize the shape
            z_bar = np.average(Z, axis=0)
            X = Z - z_bar.reshape((-1, 2))
            X = X.T @ X
            self.V = self.get_downscaled_measurement_matrix(X / (len(Z) - 1))
            self.V *= (self.v - self._d - 1)
            return self.get_state()
        else:
    ...
    def _matrix_to_params_rm(X):
        ...
        # always return semi-axis ordered in descending manner:
        if val[0] > val[1]:
            p = [alpha, *np.sqrt(val)]
        else:
            p = [alpha + np.pi / 2, *np.sqrt(val[::-1])]
    ...
    def update_rm(self, W, m_minus, P_minus, v_minus, V_minus):
        ...
        Y = self.get_scaled_shape_matrix(X_hat) + self.R
